...
public class Camera2BasicFragment extends Fragment
    ...
    /**
     * 카메라 전면 후면 아이디 값 default 전면
     */
    private int facingId = 0; //사진기 전면이 0번, 후면 1번
     ...
     */
    //카메라에서 사진을 찍으면 파이어베이스 서버 저장소에 저장
    private static class ImageUpLoader implements Runnable {
        ...
        @Override
        public void run() { // user ID를 이용해 사용자마다 이미지 저장 폴더 다르게 지정
            ...
            final StorageReference mountainImagesRef = storageRef.child("images/" + user.getUid()+ "/haha.jpg");
    ...
    private static class ImageSaver implements Runnable {
        ...
        public void run() {
            ...
            byte[] bytes = new byte[buffer.remaining()];
            buffer.get(bytes);
            FileOutputStream output = null; //인스턴트 생성
            try {
