...
public class WritingCommentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.writeCommentButton).setOnClickListener(onClickListener);
        findViewById(R.id.backButton4).setOnClickListener(onClickListener);
    }

    View.OnClickListener onClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            if (v.getId() == R.id.writeCommentButton) { //검색 버튼 눌렸을 때
                //검색 로직 추가
                myStartActivity(MainActivity.class);
            }
            else if (v.getId() == R.id.backButton4) { //뒤로가기 버튼 눌렸을 때
                myStartActivity(MainActivity.class);
            }
        }
    };
