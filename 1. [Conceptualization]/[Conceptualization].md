...
앱에  자신이  느낀  생각,  궁금한  점들을  적어  올린다.  다른  사람들은  이  글을  본  자기 생각을  공유하거나,  공감할  수  있다.  우리들은  댓글들을  보며  뿌듯함,  따듯함  등을  느끼며  교감한다.  익명이기  때문에  서로가  자신의  본모습으로  대화하며,  솔직한  자신의  심정을  털어놓는다.  정말  편한  친구가  생기는  것과  같다.  결과적으로  이  애플리케이션이  이 용자들에게  삶의  일부로  자리  잡는다면  좋겠다.

   2) Goal
      * 대부분의  사람이  지닌  스마트폰  애플리케이션으로  서비스  개발
      * 원하는  사람  누구나  이용  가능
      * 데이터베이스를  이용한  효율적인  데이터  조정
...
- Writing Post : 글 쓰기
- Search Posts : 글 검색
- View Post List : 글 리스트 보기
- View Post : 글 보기
- View My Post List : 내가 쓴 글 리스트 보기
- View Hot Post List : 인기있는 글 리스트 보기
- Writing Comment : 댓글 쓰기
...
|**Actor**|User|
| - | - |
|**Description**|사용자가 회원정보(닉네임, 전화번호, 생년월일, 주소)를 등록한다.|  
...
| - | - |
|**Approach**|사용자가 앱을 실행 후 로그인 시, ID(Email), PW를 입력 후 로그 인을 요청하면 데이터베이스에 저장된 회원 정보를 조회 후 로그 인 성공/실패 여부 확인한다.|
...
| - | - |
|**Approach**|사용자가 앱을 실행 후 회원가입 시, ID, PW, 새로 입력 후 요청 한다. ID는 중복되지 않고 Email 형식이 일치해야 한다. 비밀번호 는 6자 이상이면 회원가입에 성공한다. 그리고 회원 정보를 데이 터베이스에 저장한다.|
...
|**Purpose**|사용자가 비밀번호를 바꾸고 싶은 경우 변경한다|
| - | - |
|**Approach**|사용자가 앱을 사용하는 중 비밀번호 변경을 요청하면 비밀번호 변경 메일을 보낸다.|
|**Dynamics**|앱 사용 중 비밀번호 변경할 경우|
|**Goals**|비밀번호 변경 기능을 구현한다.|
...
|**Purpose**|회원정보 등록하기|
| - | - |
|**Approach**|로그인 후 회원정보 등록이 되어있지 않다면 회원정보 화면으로 넘어간다. 회원정보에는 닉네임, 전화번호, 생년월일, 주소와 같은 정보를 저장한다.|
|**Dynamics**|회원정보를 등록하고 싶은 경우|
|**Goals**|회원정보를 등록해 사용자의 정보를 저장할 수 있다.|
...
|**Purpose**|작성된 게시글 리스트 보기|
| - | - |
|**Approach**|홈 화면에서는 앱에 작성된 모든 게시글을 본다. 게시글은 리스트 형태로 보여진다.|
|**Dynamics**|게시글 리스트를 보는 경우|
|**Goals**|작성된 글을 모두 볼 수 있다.|
...
**Problem  #1  Android  Studio  Jave**

안드로이드  스튜디오를  통해  개발한  경험이  없다.  따라서  Java  언어를  사용해  불편함 을  없앤다.  그리고  충분한  공부를  통해  앱을  개발한다.

**Problem  #2  Firebase  :  Database**

데이터베이스를  이용해서  프로그래밍을  개발한  경험이  없다.  앱에  작성된  정보를  효율 적으로  관리하기  위해  데이터베이스  응용법을  터득해야  한다.  댓글의  개수는  계속  증가 하기  때문에,  이를  효율적으로  저장할  방법을  배워야  한다.  데이터베이스로는  파이어베 이스를  채택한다.
...
[그림  1] <https://jhealthmedia.joins.com/article/article_view.asp?pno=25382>
